...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1">
                <span class="text-muted fw-light">ยืม-คืนพัสดุ /</span> รายละเอียดคำขอยืมเลขที่ <?= esc($borrow['borrow_code']) ?>
            </h4>
            <p class="text-muted mb-0">ตรวจสอบข้อมูลผู้ยืม รายการพัสดุ และหลักฐานภาพถ่าย (สำหรับเจ้าหน้าที่)</p>
        </div>
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-outline-danger rounded-pill px-3" onclick="deleteThisBorrow()">
                <i class="bi bi-trash3 me-1"></i> ลบคำขอนี้
            </button>
            <a href="<?= base_url('Equipment/Print/' . $borrow['borrow_id']) ?>" target="_blank" class="btn btn-outline-primary rounded-pill px-3">
                <i class="bi bi-printer me-1"></i> พิมพ์ใบคำขอยืม
            </a>
            <a href="<?= base_url('Equipment/Approve') ?>" class="btn btn-outline-secondary rounded-pill px-3">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-transparent py-3">
                    <h6 class="fw-bold text-dark mb-0"><i class="bi bi-camera-fill text-primary me-2"></i>รูปถ่ายผู้ยืมคู่กับพัสดุตอนส่งมอบ (จ่ายของ)</h6>
                </div>
                <div class="card-body">
                    <?php if (!empty($borrow['pickup_photo'])): ?>
                        ...
                        $firstPickupUrl = \App\Libraries\RemoteUploadService::getUrl($firstPickup, 'Equipment/pickup');
                        ?>
                        <div class="proof-box">
                            ...
                            <div class="d-flex flex-wrap gap-2 justify-content-center mb-2">
                                <?php foreach ($pickupList as $pIdx => $pImg): 
                                    $pUrl = \App\Libraries\RemoteUploadService::getUrl($pImg, 'Equipment/pickup');
                                ?>
                                    <a href="<?= $pUrl ?>" target="_blank" class="border rounded-2 overflow-hidden shadow-sm d-inline-block" style="width: 54px; height: 54px;">
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-transparent py-3">
                    <h6 class="fw-bold text-dark mb-0"><i class="bi bi-shield-check text-success me-2"></i>รูปถ่ายผู้คืนคู่กับสภาพพัสดุตอนรับคืน</h6>
                </div>
                <div class="card-body">
                    <?php if (!empty($borrow['return_photo'])): ?>
                        ...
                        $firstReturnUrl = \App\Libraries\RemoteUploadService::getUrl($firstReturn, 'Equipment/return');
                        ?>
                        <div class="proof-box">
                            ...
                            <div class="d-flex flex-wrap gap-2 justify-content-center mb-2">
                                <?php foreach ($returnList as $rIdx => $rImg): 
                                    $rUrl = \App\Libraries\RemoteUploadService::getUrl($rImg, 'Equipment/return');
                                ?>
                                    <a href="<?= $rUrl ?>" target="_blank" class="border rounded-2 overflow-hidden shadow-sm d-inline-block" style="width: 54px; height: 54px;">
